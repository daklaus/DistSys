...
class BiddingClientServiceImpl implements BiddingClientService {
	...
	private void postLoginAction() throws IOException {
		...
		try {
			...
			if (!matcher.matches() || matcher.groupCount() != 4) {
				throw new IOException("Server response doesn't match '!ok <client-challenge> <server-challenge> <secret-key> <iv-parameter>' ");
			}
            ...
            if (!checkClientChallenge(clientChallenge, this.clientChallenge)) {
                throw new IOException("The clientChallenge generated by client doesn't match the one returned by the server");
            }
    ...
    /**
     * This method actually sends the third part of the 3-way-handshake.
     * The 3rd part just consists of the server challenge recieved from the server.
     *
     * @param serverChallenge the received server challenge from the 2nd message
     * @param secretKey       the secretKey needed for AES encryption
     * @param ivParameter     the iv-Parameter needed for AES encryption as well
     */
    private void sendServerChallenge(byte[] serverChallenge, byte[] secretKey, byte[] ivParameter) throws IOException {
        TCPClientNetworkService aesClientNetworkService = NetworkServiceFactory.newAESTCPClientNetworkService(this.rawNS, secretKey, ivParameter);
        String encodedServerChallenge = new String(Base64.encode(serverChallenge), Charset.forName("UTF-16"));
     ...
     * Base64 encoding
     *
     * @param charset the charset which will be used for translating the encoded
    ...
    /**
     * This method checks if two Base64 encoded Strings are equal.
     * If any of the two Strings are null, false is returned.
     *
     * @param ch1
     * @param ch2
	...
	public void connect() throws IOException {
		if (currentNS == null || !currentNS.isConnected()) {
			currentNS = NetworkServiceFactory.newTCPClientNetworkService(server,
					serverPort);
